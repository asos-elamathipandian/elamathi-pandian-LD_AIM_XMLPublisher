...
function generateCtrlNumber() {
  // 22-char base64 string matching resource file pattern e.g. D3alXyw0wESB6qYRXlNX9A
...
function buildAsnRcvXml({ asn, now }) {
  const ctrlNumber = generateCtrlNumber();
  const transmissionTimestamp = formatTimestamp(now);
  // Resource comment: Date can be current system time + 2 min
  const rcvDate = formatTimestamp(addSeconds(now, 4));
...
\t\t\t\t\t<Date TimeZone="UT" DateTypeCd="RCV_FRSTD">${rcvDate}</Date>
\t\t\t\t\t<Date TimeZone="UT" DateTypeCd="RCV_FNLD">${rcvDate}</Date>
...
async function writeAsnRcvFile({ asn, outputDir }) {
  const now = new Date();
  const xmlContent = buildAsnRcvXml({ asn, now });
...
module.exports = { buildAsnRcvXml, writeAsnRcvFile };
